import pytest 
from ambiguous_parsing.eval.metric import Formula
from ambiguous_parsing.generation.generate_pairs import (
    generate_pp_pairs,
    generate_unambiguous_pp,
    generate_scope_pairs,
    generate_reverse_scope_pairs,
    generate_unambiguous_scope,
    generate_conjunction_pairs,
    generate_unambiguous_conj,
    generate_bound_pronoun_pairs,
    generate_unambigous_bound_pronoun,
    generate_unambiguous_basic,
)

@pytest.fixture
def binding_example_1():
    return "exists x . exists a . exists e . exists i . man(x) AND eat(a) AND agent(a, x) AND ( ( drank(e) AND agent(e, x) ) OR ( slept(i) AND agent(i, x) ) )"

@pytest.fixture
def binding_example_2():
    return "exists a . exists e . ( drew(a) AND agent(a, Alan) ) OR ( ate(e) AND agent(e, Alan) )"

@pytest.fixture
def load_pp_pairs():
    pp_pairs = generate_pp_pairs()
    unambig_pp = generate_unambiguous_pp()
    return pp_pairs + unambig_pp

@pytest.fixture
def load_scope_pair():
    scope_pairs = generate_scope_pairs()
    reverse_scope_pairs = generate_reverse_scope_pairs()
    unambig_scope = generate_unambiguous_scope()
    return scope_pairs + reverse_scope_pairs + unambig_scope

@pytest.fixture
def load_conj_pairs():
    conj_pairs = generate_conjunction_pairs()
    unambig_conj = generate_unambiguous_conj()
    return conj_pairs + unambig_conj

@pytest.fixture
def load_bound_pairs():
    bound_pairs = generate_bound_pronoun_pairs(is_female=True)+\
                generate_bound_pronoun_pairs(is_female=False)
    unambig_bound = generate_unambigous_bound_pronoun(is_female=True) + \
                    generate_unambigous_bound_pronoun(is_female=False)
    return bound_pairs + unambig_bound

@pytest.fixture
def load_unambiguous():
    unambiguous = generate_unambiguous_basic()
    return unambiguous

def check_roundtrip(formula_str):
    formula_obj = Formula.parse_formula(formula_str) 
    generated_str_1 = formula_obj.canonicalize()
    formula_obj_2 = Formula.parse_formula(generated_str_1)
    generated_str_2 = formula_obj_2.canonicalize()
    print(generated_str_1)
    print(generated_str_2)
    assert (generated_str_1 == generated_str_2)


def test_binding_1(binding_example_1):
    check_roundtrip(binding_example_1)

def test_binding_2(binding_example_2):
    check_roundtrip(binding_example_2)

# def test_pp_pairs(load_pp_pairs):
#     for pair in load_pp_pairs:
#         check_roundtrip(pair['lf'])

# def test_scope_pairs(load_scope_pair):
#     for pair in load_scope_pair:
#         check_roundtrip(pair['lf'])

def test_conj_trouble():
    ex = "exists x . exists a . exists e . exists i . ( bird(x) AND drank(a) AND agent(a, x) AND slept(e) AND agent(e, x) ) OR ( ate(i) AND agent(i, x) )"
    check_roundtrip(ex)

# TODO (elias): Get this test passing, the hardest one wrt ordering 
def test_conj_pairs(load_conj_pairs):
    skipped = 0
    for pair in load_conj_pairs:
        # print(pair['lf'])
        try:
            check_roundtrip(pair['lf'])
        except AssertionError:
            print(pair['lf'])
            assert(1==2)
            skipped += 1
    print(f"Skipped {skipped} pairs")
    assert(1==2)

# def test_bound_pairs(load_bound_pairs):
#     for pair in load_bound_pairs:
#         check_roundtrip(pair['lf'])

# def test_unambiguous(load_unambiguous):
#     for pair in load_unambiguous:
#         check_roundtrip(pair['lf'])
